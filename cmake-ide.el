...
(defun cide--merge-paired-args (str)
  "Concatenates two arguments if the first argument ends with '='."
  (let ((result nil) (args (split-string (car str))))
    (while args
	(if (< (length args) 2)
	    (progn (setq result (append result args)) (setq args nil))
	    (let ((a1 (car args)) (a2 (nth 1 args)))
	    (if (string-match-p "=$" a1)
		(progn (setq result (append result (list (concat a1 a2)))) (setq args (nthcdr 2 args)))
		(progn (setq result (append result (list a1))) (setq args (nthcdr 1 args)))
		))))
    (list (s-join " " result))))
...
(defun cide--idb-all-commands (idb)
  ...
  (cide--merge-paired-args (mapcar (lambda (x) (s-join " " (cide--file-params-to-args x))) (cide--idb-all-objs idb))))
